...
export default LikeListScreen = ({ route, navigation }) => {
    ...
    const likeList = useSelector((state) => state.getPostLike.getPostLikeDetails)
    const followList = useSelector((state) => state.getfollow.getfollowDetails)
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <Header color="#131128" title="Likes" />
            <FlatList
                data={likeList === undefined ? [] : likeList.data}
                renderItem={({ item, key }) => {
                    return (
                        <>
                            <TouchableOpacity
