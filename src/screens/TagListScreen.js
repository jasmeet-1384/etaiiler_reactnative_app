...
import { getPostSharesUrl, getPostTagsUrl } from '../api/apiConstant'
import FollowUnfollow from '../components/FollowUnfollow'


export default TagListScreen = ({ route , navigation }) => {
    ...
    const getPostSharesFunction = async (postId) => {
        try {
            ...
            var response = await fetch(getPostTagsUrl, {
                method: 'POST',
                headers: {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <Header color="#131128" title="TagList" />
            <FlatList
                data={followArr}
                renderItem={({ item, key }) => {
                    return (
                        <>
                            {
                                item.user_id == undefined ? null : <TouchableOpacity
