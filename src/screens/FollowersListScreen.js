...
import { getFollowersListUrl, getPostSharesUrl } from '../api/apiConstant'
import FollowUnfollow from '../components/FollowUnfollow'
import ActivityIndicatorComponent from '../components/ActivityIndicatorComponent'

export default FollowerListScreen = ({route,navigation}) => {
    ...
    const getPostSharesFunction = async () => {
        try {
            ...
            var response = await fetch(getFollowersListUrl, {
                method: 'POST',
                headers: {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <Header color="#131128" title="Followers" />
            {
